...
const Todo = require("../models/Favorites");
...
userRouter.post(
  ...
  async (req, res) => {
    try {
      const favorite = new Todo(req.body); //create new model
      await favorite.save();
      req.user.favorites.push(favorite); //push todoID
      try {
    ...
    User.findById({ _id: req.user._id })
      ...
      .exec((err, document) => {
        if (err) {
          ...
          res
            .status(200)
            .json({ favorites: document.favorites, authenticated: true });
      });
  }
);
userRouter.get(
